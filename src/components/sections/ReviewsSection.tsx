...
export default function ReviewsSection() {
  ...
  return (
    <section ref={sectionRef} style={{
      ...
      {/* Warm glow */}
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 15% 50%, rgba(201,150,62,0.07) 0%, transparent 55%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '1px', background: 'linear-gradient(to right, transparent, rgba(201,150,62,0.2), transparent)' }} />
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '64px', flexWrap: 'wrap', gap: '32px' }}>
          ...
          <div className="fade-up" style={{ transitionDelay: '100ms', alignSelf: 'center' }}>
            <div style={{
              ...
              boxShadow: '0 4px 24px rgba(28,16,8,0.08)',
            }}>
            ...
            return (
              <div key={review.id} className="fade-up" style={{ transitionDelay: `${i * 100}ms` }}>
                <div style={{
                  ...
                  border: `1px solid ${isHighlighted ? 'rgba(201,150,62,0.35)' : 'rgba(28,16,8,0.09)'}`,
                  borderRadius: '4px',
                  padding: '32px 28px',
                  height: '100%',
                  display: 'flex',
                  flexDirection: 'column',
                  boxShadow: isHighlighted ? '0 4px 28px rgba(201,150,62,0.1)' : '0 2px 12px rgba(28,16,8,0.06)',
                  transition: 'box-shadow 300ms, border-color 300ms, transform 300ms',
                }}
                  onMouseEnter={e => {
                    const el = e.currentTarget as HTMLElement
                    el.style.boxShadow = '0 20px 60px rgba(28,16,8,0.14)'
                    el.style.borderColor = 'rgba(201,150,62,0.4)'
                    el.style.transform = 'translateY(-4px)'
                  }}
                  onMouseLeave={e => {
                    const el = e.currentTarget as HTMLElement
                    el.style.boxShadow = isHighlighted ? '0 4px 28px rgba(201,150,62,0.1)' : '0 2px 12px rgba(28,16,8,0.06)'
                    el.style.borderColor = isHighlighted ? 'rgba(201,150,62,0.35)' : 'rgba(28,16,8,0.09)'
                ...
                >
                  ...
                  {/* Reviewer */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderTop: '1px solid rgba(28,16,8,0.08)', paddingTop: '20px' }}>
                    <div style={{
                      width: '40px', height: '40px', borderRadius: '50%',
                      background: isHighlighted ? 'rgba(201,150,62,0.15)' : 'rgba(28,16,8,0.06)',
                      border: `1px solid ${isHighlighted ? 'rgba(201,150,62,0.3)' : 'rgba(28,16,8,0.1)'}`,
